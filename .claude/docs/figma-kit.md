...
## The OG Sections page, 2026-09-06 — and two read traps

- **Page `61:8` ("07 · OG Sections — fill per board")** is where the og sheets live now: Dylan's own canvases for OG 03/04/05/06 and one in the OG 01 slot that is really Bluepiro (see lightbox.md's OG 06 COLLIDE entry); OG 02 still holds the D cluster seed. Claude's collide sheet for OG 06 is `228:65` at (5300, 2400), under his. Dozens of loose `IMG_7xxx` rectangles (the angled shoot) float around the row — place new canvases at y ≥ 2400 or x ≥ 8400 to stay clear. Style tokens measured off his OG 06 (`61:25`): white frame, guide rect `61:26` (1px `#999` dashed 8/8, INSIDE), photo tiles radius 6 + drop shadow blur 4 offset 10/10 alpha .25, Inter Regular 20, arrows = VECTOR halves with a per-vertex `ARROW_LINES` cap (clone `164:796`/`164:798`, then `resize(w, 0)`; the rotated-180 half extends LEFT from its origin).
- **Reads that touch certain nodes die in the ~20KB SSE truncation no matter how small the return** — every `use_figma` read of the OG 05 frame `204:77` (even a names-only 8-child slice), the OG 01 frame `61:10`, and children 13–17 of the "Originals LB Mockup" page failed; `get_metadata` on those pages too. The count-only read of the same page succeeds, so it isn't the envelope — something on those nodes inflates the response. Don't chase it: `get_screenshot` / `download_assets` on the same ids work fine, and the tokens are readable off a sibling sheet.
- **Cloning across parents is the cheap composition path:** `src.clone()` lands beside the source; `frame.appendChild(clone)` reparents; `rescale(k)` keeps a CROP fill's crop (a `resize` that changes the ratio stretches it); position rotated clones through `absoluteBoundingBox` deltas, never `x/y`. `download_assets` at `defaultScale: 1` exports the 960×1280 frame as the zone's sheet directly.
- **Exporting tiles for the site (2026-09-06 recipe):** clone the photo rectangles into a temporary staging frame (`effects = []`, `cornerRadius = 0`, `strokes = []`, `rotation = 0`), `download_assets` each clone with `defaultFormat: "jpg", defaultScale: 2` (a 400×300 canvas crop → 800×600 jpg, 200–600KB), curl into `site/img/og/`, then `remove()` the staging frame. `rawImages` in the same response are the full-res source uploads — for OG 02 the launch-week close-ups came out that way (1200×900) and beat the tiny D-cluster crops.
