...
**THE SITE IS LIVE at https://avrg.cards (launched 2026-08-22).** GH Pages serves `main` here with Enforce HTTPS on; `avrg.website` 301-forwards to it (Porkbun-side). The vault repo is **private** and its old Pages URL is dead. Every push to main publishes immediately.
...
- [k-core.md](.claude/docs/k-core.md) — page-level: the one light ground (dark retired 2026-08-20), pinned CARD/LB constants, light-mode plumbing, the drawn logo.
...
- [sounds.md](.claude/docs/sounds.md) — the synth kit + THE GATE (phone silence at 940, per-voice cooldown, the add-suppression), settle-sync, and the trigger inventory. Read before adding or moving any sound.
...
- The z ladder: shields (deal + menu-select + intro) 6700 · **phone pick-flight 6650** (inline on the fly in `pickCatch` — Dylan's 2026-08-22 ruling: the phone add rides OVER the bar and lands on it; the bar was cutting it in half mid-air) · topbar 6600 · flights (`.bay-fly`) 6500 · intro masthead lift 6450 · focus veil (`.fveil`) 6400 · bay 5500 · lb 5000 · `#vlb` 7000. The bar rides ABOVE the desktop flights on purpose; `#vlb` is the one true full-screen modal. New overlays pick a rung deliberately.
