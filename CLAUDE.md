...
- **2026-08-16: v1 was created** from the vault at commit `9aca992` (working tree clean). Two commits so far: THE COPY (the live site, whole, in its own house) and THE DEAD CODE COMES OUT (the takeover-case machine — `openCase` family, `mopen`/`case-fade` CSS — and `cardInner`'s photo branch, both unreachable, removed for real). Only comments still name them.
