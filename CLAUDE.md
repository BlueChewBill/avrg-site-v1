...
- **The og lightbox's right column is a manifest, not markup:** `OG_SHEETS` (a baked, hand-composed sheet per ref) outranks `OG_TPL` (the live template: four photo seats + three spec strings per ref). Both hand-authored beside `DIMS_MM`; a dash on a live sheet is an empty seat for Dylan, never a guess. Record: [lightbox.md](.claude/docs/lightbox.md), THE OG TEMPLATE.
