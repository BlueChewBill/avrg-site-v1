...
*Derived 2026-08-28 · commit 0ff5973 · index.html 16685 lines*
...
- `JCARDS (framev3 template)` (index.html:6996) — the ONE CARD markup — every card on the site prints from JCARDS[CARD]; CARD is pinned to "framev3"
- `jmeta` (index.html:6929) — shapes a data.js board into card meta (acc, ref, dims, cutouts)
- `cardInner / renderColPage` (index.html:7824) — collection-page grids (.scard slots)
- `bindScards` (index.html:7808) — grid slot wiring: click/keyboard -> openLb
- `renderLisst` (index.html:8148) — YOUR PICKS page cards (one producer for both dresses: YOUR PICKS on the dock side, My Lisst on desktop)
- `renderBay` (index.html:12717) — draws the drawer/bay shelf cards — the shelf's producer (flyToBay is only the flight)
- `the shop conveyor` (index.html:11999) — home belt cards (recycler owns their visibility)
- `flyToBay` (index.html:15288) — card -> drawer flight
- `setFlip / flipStage` (index.html:15858) — the flip system (chip = the hidden-face mini)
- `faceSync + FACE` (index.html:6971) — face memory: cards inherit the last-seen side
- `lockCardScale` (index.html:7791) — grid render law: 232px then transform-down
- `migrateHole` (index.html:10961) — lb exchange re-seats the grid hole on every landing
- `cardDressOn / cardDressOff` (index.html:6634) — the hover decode pair: name<->dims, ref<->AVRG, status in/out — display is a TEXT WRITE, never CSS
- `stayLand / dockDress` (index.html:6607) — the static text lands (no hover to earn a decode) — note: stayLand resolves dims||name, so THE LB CARD ALREADY RESTS ON DIMS on desktop
- `DRESS_TEXTS` (index.html:6562) — the four text slots the dress systems own (.fc-ref .ft, .nmt, .sttxt, .fc-list .ft)
- `DIMS_MM` (index.html:6883) — hand-authored real dims (data.js is generated, so these live in-page)
- `CANVA / INVREF` (index.html:6778) — cutout map + canonical inventory refs (the jref/refOf law)
