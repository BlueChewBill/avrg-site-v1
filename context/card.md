...
*Derived 2026-09-05 · commit 69f60ab · index.html 17391 lines*
...
- `JCARDS (framev3 template)` (index.html:7335) — the ONE CARD markup — every card on the site prints from JCARDS[CARD]; CARD is pinned to "framev3"
- `jmeta` (index.html:7191) — shapes a data.js board into card meta (acc, ref, dims, cutouts)
- `cardInner / renderColPage` (index.html:8169) — collection-page grids (.scard slots)
- `bindScards` (index.html:8153) — grid slot wiring: click/keyboard -> openLb
- `renderLisst` (index.html:8501) — YOUR PICKS page cards (one producer for both dresses: YOUR PICKS on the dock side, My Lisst on desktop)
- `renderBay` (index.html:13373) — draws the drawer/bay shelf cards — the shelf's producer (flyToBay is only the flight)
- `the shop conveyor` (index.html:12655) — home belt cards (recycler owns their visibility)
- `flyToBay` (index.html:15996) — card -> drawer flight
- `setFlip / flipStage` (index.html:16570) — the flip system (chip = the hidden-face mini)
- `faceSync + FACE` (index.html:7305) — face memory: cards inherit the last-seen side
- `lockCardScale` (index.html:8136) — grid render law: 232px then transform-down
- `migrateHole` (index.html:11461) — lb exchange re-seats the grid hole on every landing
