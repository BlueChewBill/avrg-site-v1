...
*Derived 2026-09-11 · commit a314c01 · index.html 18007 lines*
...
- `JCARDS (framev3 template)` (index.html:7698) — the ONE CARD markup — every card on the site prints from JCARDS[CARD]; CARD is pinned to "framev3"
- `jmeta` (index.html:7537) — shapes a data.js board into card meta (acc, ref, dims, cutouts)
- `cardInner / renderColPage` (index.html:8532) — collection-page grids (.scard slots)
- `bindScards` (index.html:8516) — grid slot wiring: click/keyboard -> openLb
- `renderLisst` (index.html:8864) — YOUR PICKS page cards (one producer for both dresses: YOUR PICKS on the dock side, My Lisst on desktop)
- `renderBay` (index.html:13717) — draws the drawer/bay shelf cards — the shelf's producer (flyToBay is only the flight)
- `the shop conveyor` (index.html:12998) — home belt cards (recycler owns their visibility)
- `flyToBay` (index.html:16553) — card -> drawer flight
- `setFlip / flipStage` (index.html:17145) — the flip system (chip = the hidden-face mini)
- `faceSync + FACE` (index.html:7668) — face memory: cards inherit the last-seen side
- `lockCardScale` (index.html:8499) — grid render law: 232px then transform-down
- `migrateHole` (index.html:11803) — lb exchange re-seats the grid hole on every landing
- `cardDressOn / cardDressOff` (index.html:7127) — the hover decode pair: name<->dims, ref<->AVRG, status in/out — display is a TEXT WRITE, never CSS
- `stayLand / dockDress` (index.html:7100) — the static text lands (no hover to earn a decode) — note: stayLand resolves dims||name, so THE LB CARD ALREADY RESTS ON DIMS on desktop
- `DRESS_TEXTS` (index.html:7044) — the four text slots the dress systems own (.fc-ref .ft, .nmt, .sttxt, .fc-list .ft)
- `DIMS_MM` (index.html:7404) — hand-authored real dims (data.js is generated, so these live in-page)
- `CANVA / INVREF` (index.html:7255) — cutout map + canonical inventory refs (the jref/refOf law)
...
- `(prefers-reduced-motion: reduce)` ×9
- `(max-width: 720px)` ×4
- `(min-width: 721px)` ×2
