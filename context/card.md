...
*Derived 2026-08-28 · commit ed3b679 · index.html 16668 lines*
...
- `JCARDS (framev3 template)` (index.html:6968) — the ONE CARD markup — every card on the site prints from JCARDS[CARD]; CARD is pinned to "framev3"
- `jmeta` (index.html:6901) — shapes a data.js board into card meta (acc, ref, dims, cutouts)
- `cardInner / renderColPage` (index.html:7796) — collection-page grids (.scard slots)
- `bindScards` (index.html:7780) — grid slot wiring: click/keyboard -> openLb
- `renderLisst` (index.html:8120) — YOUR PICKS page cards (one producer for both dresses: YOUR PICKS on the dock side, My Lisst on desktop)
- `renderBay` (index.html:12692) — draws the drawer/bay shelf cards — the shelf's producer (flyToBay is only the flight)
- `the shop conveyor` (index.html:11974) — home belt cards (recycler owns their visibility)
- `flyToBay` (index.html:15265) — card -> drawer flight
- `setFlip / flipStage` (index.html:15835) — the flip system (chip = the hidden-face mini)
- `faceSync + FACE` (index.html:6943) — face memory: cards inherit the last-seen side
- `lockCardScale` (index.html:7763) — grid render law: 232px then transform-down
- `migrateHole` (index.html:10951) — lb exchange re-seats the grid hole on every landing
- `cardDressOn / cardDressOff` (index.html:6606) — the hover decode pair: name<->dims, ref<->AVRG, status in/out — display is a TEXT WRITE, never CSS
- `stayLand / dockDress` (index.html:6575) — the static text lands (no hover to earn a decode) — note: stayLand resolves dims||name, so THE LB CARD ALREADY RESTS ON DIMS on desktop
- `DRESS_TEXTS` (index.html:6530) — the four text slots the dress systems own (.fc-ref .ft, .nmt, .sttxt, .fc-list .ft)
- `DIMS_MM` (index.html:6855) — hand-authored real dims (data.js is generated, so these live in-page)
- `CANVA / INVREF` (index.html:6750) — cutout map + canonical inventory refs (the jref/refOf law)
