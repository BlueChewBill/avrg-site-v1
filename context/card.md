...
- `jmeta` (index.html:7398) — shapes a data.js board into card meta (acc, ref, dims, cutouts)
- `cardInner / renderColPage` (index.html:8393) — collection-page grids (.scard slots)
- `bindScards` (index.html:8377) — grid slot wiring: click/keyboard -> openLb
- `renderLisst` (index.html:8725) — YOUR PICKS page cards (one producer for both dresses: YOUR PICKS on the dock side, My Lisst on desktop)
- `renderBay` (index.html:13600) — draws the drawer/bay shelf cards — the shelf's producer (flyToBay is only the flight)
- `the shop conveyor` (index.html:12882) — home belt cards (recycler owns their visibility)
- `flyToBay` (index.html:16223) — card -> drawer flight
- `setFlip / flipStage` (index.html:16797) — the flip system (chip = the hidden-face mini)
- `faceSync + FACE` (index.html:7529) — face memory: cards inherit the last-seen side
- `lockCardScale` (index.html:8360) — grid render law: 232px then transform-down
- `migrateHole` (index.html:11691) — lb exchange re-seats the grid hole on every landing
- `cardDressOn / cardDressOff` (index.html:6988) — the hover decode pair: name<->dims, ref<->AVRG, status in/out — display is a TEXT WRITE, never CSS
- `stayLand / dockDress` (index.html:6961) — the static text lands (no hover to earn a decode) — note: stayLand resolves dims||name, so THE LB CARD ALREADY RESTS ON DIMS on desktop
- `DRESS_TEXTS` (index.html:6905) — the four text slots the dress systems own (.fc-ref .ft, .nmt, .sttxt, .fc-list .ft)
- `DIMS_MM` (index.html:7265) — hand-authored real dims (data.js is generated, so these live in-page)
- `CANVA / INVREF` (index.html:7116) — cutout map + canonical inventory refs (the jref/refOf law)
...
- `(hover: hover)` keeps the phone out; the card's own hover still draws the seat. */ .d7f .fc-list .ft` ×1
